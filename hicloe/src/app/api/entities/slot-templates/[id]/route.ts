...
export const PATCH = guarded("entities:write", async (ctx, params) => {
  ...
  await db.$transaction(async (tx) => {
    if (parsed.data.active) await tx.slotTemplate.updateMany({ data: { active: false } });
...
export const DELETE = guarded("entities:write", async (ctx, params) => {
  ...
  await db.$transaction(async (tx) => {
    await tx.slotTemplate.delete({ where: { id } });
    await audit(
      { action: "slotTemplate.deleted", actorId: ctx.user.id, entityType: "SlotTemplate", entityId: id, before: { name: before.name }, requestId: ctx.requestId, ip: ctx.ip, userAgent: ctx.userAgent },
      tx
    );
  });
  return NextResponse.json({ ok: true });
